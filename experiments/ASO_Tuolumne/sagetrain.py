...
CONSOLE_JOB_URL = (
    "https://{region}.console.aws.amazon.com/sagemaker/home?region={region}#/jobs/{job}"
)
...
def build_estimator(session, prefix, sagemaker_image, sage_role_arn, output_bucket):
    ...
    e = sagemaker.estimator.Estimator(
        sagemaker_image,
        sage_role_arn,
        1,
        "ml.p2.xlarge",
...
@click.option(
    "--max_runtime", "max_runtime", required=True, default=(60 * 60 * 10)
)  # 10 hours
def sage_train(
    config,
    sagemaker_image,
    ...
    e.fit({"config": config_location}, wait=False)
