...
@click.option("--footprint",
             help="Data footprint geojson (within <destination>!).", required=True)
...
def get_modis(destination, date, search_window, footprint, choose, clip, reproject):
    ...
    results = cmrAPI.short_name(MODIS_SNOW_CMR_SHORT_NAME)\
                  .bounding_box(*searchFootprint.bounds)\
                  .temporal(*date_range).get()
    ...
    MODIS_DL_COMMAND = "wget "\
        ...
        "{image_url}"\
    .format(destination=workdir, image_url = image_url)
    ...
    MODIS_CONVERT_COMMAND = "gdalwarp "\
